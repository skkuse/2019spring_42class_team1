    ...
    def add_censors(self, censors, start_millis, end_millis):
        for censor in censors:
            if not censor['label'] in ['F_BREAST', 'F_GENITALIA', 'M_GENETALIA']:
                continue
            blurbox = censor['box']
            self.add_bp(BlurPoint(x=blurbox[0], y=blurbox[1], width=blurbox[2] - blurbox[0],
                                  height=blurbox[3] - blurbox[1], start_millis=start_millis, end_millis=end_millis))
    ...
    def list_for_overlay(self):
        ...
        for key in self.pool.keys():
            ...
            for (idx, value) in enumerate(values):
                ...
                if idx == 0:
                    overlay['enable'] = 'between(t, %f, %f)' % (
                        value.start_millis / 1000, value.end_millis / 1000)
                else:
                    overlay['enable'] += '+between(t, %f, %f)' % (
                        value.start_millis / 1000, value.end_millis / 1000)
...
def detect(src_path, video):
    ...
    for frame in sorted(os.listdir(output_dir), key=lambda f: frame_order(f)):
        ...
        if nudity_prob > 0.8:
            ...
            if not detected:
                detected.append(DetectedScene(src_video=video, start_millis=start_millis,
                                              end_millis=end_millis, cause=DetectedScene.DetectionCause.NUDITY))
            else:
                latest = detected[-1]
        ...
        try:
            ...
            for overlay in bps.list_for_overlay():
                blur_op = blur_op.overlay(
                    infile.crop(x=overlay['x'], y=overlay['y'], width=overlay['width'], height=overlay['height']).filter_(
...
def extract_frames(src_path, outdir_path, frame_size, interval_millis=1000):
    ...
    fwidth, fheight = video.get(cv2.CAP_PROP_FRAME_WIDTH), video.get(
        cv2.CAP_PROP_FRAME_HEIGHT)
    fps = video.get(cv2.CAP_PROP_FPS)
    num_frames = video.get(cv2.CAP_PROP_FRAME_COUNT)
    print('###### %d x %d, fps: %f, the number of frames: %d' %
          (fwidth, fheight, fps, num_frames))
    ...
    while(True):
        ...
        if ret:
            frame_path = os.path.join(
                outdir_path, 'frame_' + str(cur_frame) + '.jpg')
    ...
    video.release()
    cv2.destroyAllWindows()
    print('###### extracting frames complete from %s to %s complete.' %
          (src_path, outdir_path))
